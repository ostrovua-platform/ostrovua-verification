...
Після успішної перевірки на сервер іде рівно один запит:

```json
POST /auth/verify/approve
{ "method": "nfc_passport", "challengeId": "…" }
```

Жодного поля документа. Сервер зберігає лише три речі:
`verified: true`, дату та метод перевірки. Це видно в
`Sources/Attestation/VerifyService.swift` і в серверному обробнику
(`docs/server-side.md`).
...
3. **Face check** (`Sources/Verification/Camera`) — звірка обличчя
   ...
   depth/LiDAR — перевірка живої присутності. Ніщо з цього не
   передається і не зберігається.
