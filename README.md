...
> **Чесна межа й мінімізація.** Хеш DG — технічний ідентифікатор
> примірника документа (дані з нього не відновити). Мінімізуємо
> (аудит #12): телефон надсилає РІВНО ОДИН хеш на групу (SHA-256,
> яким підписані українські SOD), а не набір «про всяк випадок».
> Сервер перевіряє ним PA і НЕ зберігає хеш; зберігається лише
> односторонній токен `HMAC(pepper, хеш DG1)` — щоб один паспорт не
> верифікував кілька акаунтів і щоб бан документа був незворотним
> (pepper — поза базою, відновити номер/імʼя неможливо). PA доводить
> справжність ДАНИХ чипа, але не ловить точну копію чипа (клон) —
> для цього існує Chip/Active Authentication, наступний етап
